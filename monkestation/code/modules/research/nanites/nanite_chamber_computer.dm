...
/obj/machinery/computer/nanite_chamber_control/machine_ui_act(action, list/params, mob/user, ai_called = FALSE)
	...
	switch(action)
		...
		if("set_safety")
			...
			if(!isnull(threshold))
				...
				chamber.occupant.investigate_log("'s nanites' safety threshold was set to [threshold] by [key_name(user)] via [src] at [AREACOORD(src)].", INVESTIGATE_NANITES)
			. = TRUE
		if("set_cloud")
			...
			if(!isnull(cloud_id))
				...
				playsound(src, "terminal_type", 25, FALSE)
				chamber.occupant.investigate_log("'s nanites' cloud id was set to [cloud_id] by [key_name(user)] via [src] at [AREACOORD(src)].", INVESTIGATE_NANITES)
			. = TRUE
		if("connect_chamber")
			find_chamber()
			. = TRUE
		if("remove_nanites")
			...
			log_combat(user, chamber.occupant, "cleared nanites from", null, "via [src]")
			chamber.occupant.investigate_log("'s nanites were cleared by [key_name(user)] via [src] at [AREACOORD(src)].", INVESTIGATE_NANITES)
			. = TRUE
		if("nanite_injection")
			playsound(src, 'sound/machines/terminal_prompt.ogg', 25, FALSE)
			chamber.inject_nanites()
			log_combat(user, chamber.occupant, "injected", null, "with nanites via [src]")
			chamber.occupant.investigate_log("was injected with nanites by [key_name(user)] via [src] at [AREACOORD(src)].", INVESTIGATE_NANITES)
			. = TRUE
