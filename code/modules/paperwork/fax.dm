...
/obj/machinery/fax/machine_ui_act(action, list/params, mob/user, ai_called = FALSE)
	...
	switch(action)
		...
		if("send_special")
			...
			if(!istype(fax_paper))
				to_chat(user, icon2html(src.icon, user) + span_warning("Fax can only send paper on this protected network, sorry.")) // MONKESTATION EDIT: cannot send all above > can only send
				return
			...
			GLOB.requests.fax_request(user.client, "sent a fax message from [fax_name]/[fax_id] to [params["name"]]", fax_paper)
			to_chat(GLOB.admins, span_adminnotice("[icon2html(src.icon, GLOB.admins)]<b><font color=green>FAX REQUEST: </font>[ADMIN_FULLMONTY(user)]:</b> [span_linkify("sent a fax message from [fax_name]/[fax_id][ADMIN_FLW(src)] to [html_encode(params["name"])]")] [ADMIN_SHOW_PAPER(fax_paper)]"), confidential = TRUE)
